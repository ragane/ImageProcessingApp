...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        self.ImageProcessingBox.setGeometry(QtCore.QRect(650, 10, 781, 591))
        self.ImageProcessingBox.setObjectName("ImageProcessingBox")

        '''
        self.Histogram = QtWidgets.QLabel(self.ImageProcessingBox)
        self.Histogram.setGeometry(QtCore.QRect(12, 338, 551, 241))
        self.Histogram.setObjectName("Histogram")
        self.Histogram = FigureCanvas(Figure())
        self.Histogram.sumbu1= self.Histogram.figure.add_subplot(111)
        self.Histogram.figure.set_facecolor("xkcd:wheat")
        '''

        self.layoutWidget = QtWidgets.QWidget(self.ImageProcessingBox)
        self.layoutWidget.setGeometry(QtCore.QRect(14, 31, 680, 296))
        ...
        self.FileMenu.setGeometry(QtCore.QRect(0, 0, 1700, 21))
        ...
        self.MedBlurrButton.clicked.connect(self.ImgDispLabel.clear) # type: ignore
        self.BillateralButton.clicked.connect(self.ImgDispLabel.clear) # type: ignore
        self.GaussBlurrButton.clicked.connect(self.ImgDispLabel.clear) # type: ignore
        self.ImgBlurrButton.clicked.connect(self.ImgDispLabel.clear) # type: ignore
        self.ConvButton.clicked.connect(self.ImgDispLabel.clear) # type: ignore
    ...
    ''' Function used to load images selected by user and set it to label '''
    def loadImage(self):
    ...
    ''' Function used to resize selected images and convert them to QImage '''
    ...
    ''' Function used to save processed image by menu '''
    def saveImage(self):
        cv2.imwrite(self.filename, self.image)
        print("Saved: ", self.filename)

    ''' Function used to save processed image with name and location selected by user by menu '''
    ...
    ''' Function used to close app by menu '''
    def CloseApp(self):
        print("Application is being closed.")
        app.quit()

    ''' Function used to select and use proper geometric transformations on processed image '''
    ...
    ''' class used to set thresh of image appropriate to value and threshing type selected by user '''
    ...
    ''' Function used by brightness functions to update image '''
    def UpdateImg(self):
        image = self.SetBrightValue(self.image, self.brightness_val_now)
        self.setImage(image)

    ''' Function used to set colorspace of image chosen by operator '''
    ...
    ''' Function used to check which thresholding type is selected and set it '''
